...
got wrong**, which is the half a commit message never carries. The Shimeji phase log keeps every
original estimate beside its correction — "Left below as written, with a note on each, because what
the estimates got wrong is the useful part" — and the `activeIE` headline of 335 lost actions is
preserved next to the survey showing it never meant what it appeared to mean.

**Read every entry as of its own date.** Version numbers, and the words "current", "next" and "still
open", describe the state when the entry was written. The product was rebased to 1.0.0 in a fresh
...
### Bugs & maintenance — closed entries
...
*(The two 📌 open items from this section — `--module-selftest=<id>` picking the first `SelfTest` in
the assembly, and the vestigial `microsoft/setup-msbuild` in `release.yml` — stay in
[`../BACKLOG.md`](../BACKLOG.md) under "Bugs & maintenance", which is where `handoff.md` points at
them.)*
...
- **Still genuinely missing, and open:** MOVING a companion ("walk to centre screen"). Recorded in
  [`../BACKLOG.md`](../BACKLOG.md), which also carries the reason it is bigger than it sounds.
...
Diarization (speaker labels) and refreshing the device dropdowns without an app restart are open in
[`../BACKLOG.md`](../BACKLOG.md); the live recording smoke test is in [`BLOCKED.md`](BLOCKED.md).
...
frame-heavy skins; the runtime still caps on-screen frames at 256 px (the memory guard is unchanged).
...
### Known ABI gaps — the one that closed (v1.6.0)
...
*(The third-party module ecosystem, Phase B, is the one item from this section that was never built;
it stays open in [`../BACKLOG.md`](../BACKLOG.md).)*
...
releasing is now `git tag vX.Y.Z` (see [`docs/RELEASE-CHECKLIST.md`](RELEASE-CHECKLIST.md)).
...
*(Three threads this left open — the runaway-emoji defect a live run exposed, the pane that cannot
preview an unapplied dropdown value, and auditioning the whole catalog side by side — are in
